...
# Static route w VPN connection (AWS side)
...
# -------------------------
# KROK 1: Trasa w VPC route table (konieczne, żeby AWS odsyłał ruch do LAN)
# Private route table: 192.168.22.0/24 -> VGW
# -------------------------
...
# -------------------------
# KROK 2: Security Group EC2 (ICMP + access z on-prem)
# -------------------------
resource "aws_security_group" "vpn_only" {
  ...
  ingress {
    description = "Access from on-prem"
    from_port   = 0
    to_port     = 65535
    protocol    = "tcp"
...
# -------------------------
# OUTPUTS (ułatwia weryfikację w AWS)
# -------------------------
output "vpc_id" {
